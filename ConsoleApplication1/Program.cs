                ...
                using (var site = new SPSite(siteUrl))
                {
                    ...
                    using (var web = site.RootWeb)
                    {
                        ...
                        if (list == null)
                        {
                            ...
                            var listGuid = site.RootWeb.Lists.Add(listName, listDescription,SPListTemplateType.GenericList);

                            // getting newly created list into object
                            list = site.RootWeb.Lists[listGuid];

                            // adding choice field to store from currency name
                            list.Fields.Add(fromCurrencyFieldName, SPFieldType.Choice, true);

                            // getting newly created from currency field into object
                            var spFromCurrencyField = (SPFieldChoice) list.Fields[fromCurrencyFieldName];

                            // adding choice field to store 'to currency' name
                            list.Fields.Add(toCurrencyFieldName, SPFieldType.Choice, true);

                            // getting newly created 'to currency' field into object
                            var spToCurrencyField = (SPFieldChoice) list.Fields[toCurrencyFieldName];
                            ...
                            list.Fields.Add(rateFieldName, SPFieldType.Number, true);

                            // getting newly created number field and moving into an object
                            var rateField = (SPFieldNumber) list.Fields[rateFieldName];
                        ...
                        for (var primaryIndex = 0; primaryIndex < currencies.Length; primaryIndex++)
                        {
                            ...
                            var primaryCurrency = (Currency) Enum.Parse(typeof (Currency), currencies[primaryIndex]);

                            // secondary iteration for rest of the currencies
                            for (var secondaryIndex = primaryIndex + 1;secondaryIndex < currencies.Length;secondaryIndex++)
                            {
                                // getting the currency from the current iterated element
                                var secondaryCurrency = (Currency) Enum.Parse(typeof (Currency), currencies[primaryIndex]);
                                ...
                                if (rate > 0)
                                {
                                    ...
                                    newListItem[fromCurrencyFieldName] = primaryCurrency.ToString();

                                    // updating secondary currency field value into 'to currency' field
                                    newListItem[toCurrencyFieldName] = secondaryCurrency.ToString();
