...
namespace ProjeqzCurrencyConverter
{
    public partial class CurrencyConverter
    {
        ...
        {
            if (!string.IsNullOrEmpty(CmbCurrency.Text) && !string.IsNullOrEmpty(TxtCurrencyRate.Text))
            {
                ...
                try
                {
                    ...
                    foreach (Task task in app.ActiveProject.Tasks)
                    {
                        flag = true;
                        Double cost = 0.0f;
                        var value = task.GetField(app.FieldNameToFieldConstant(CustomFieldName, PjFieldType.pjTask));
                        try
                        {
                        ...
                        task.SetField(app.FieldNameToFieldConstant(CustomFieldName, PjFieldType.pjTask),
                                      cost.ToString());
                    }
                    if (flag)
                    {
                        NotificationIcon.BalloonTipIcon = ToolTipIcon.Info;
                        NotificationIcon.BalloonTipText = @"Converting process completed successfully.";
                ...
                catch (Exception ex)
                {
                    ...
                    // GrpConverter.Label = ex.Message;
                    NotificationIcon.BalloonTipIcon = ToolTipIcon.Error;
                    NotificationIcon.BalloonTipText = ex.Message;
                    NotificationIcon.Visible = true;
                    NotificationIcon.ShowBalloonTip(10000);
                }
            }
